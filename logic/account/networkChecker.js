...
module.exports = {
  startNetworkChecking: function (redisClient, accountHashID, ipAddress, networkModel, callback) {
    module.exports.checkAccountNetworkChangeByIPAddress(redisClient, accountHashID, ipAddress, function (err, replies) {
      if (err) {
        callback(err, null)
        return
      }
      module.exports.checkAccountNetworkChangeByCountry(redisClient, accountHashID, networkModel.country, function (err, replies) {
        if (err) {
          callback(err, null)
          return
        }
        module.exports.checkAccountNetworkChangeByMacAddress(redisClient, accountHashID, networkModel.macAddress, function (err, replies) {
          if (err) {
            callback(err, null)
            return
          }
